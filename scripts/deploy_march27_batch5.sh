#!/bin/bash
# Deploy trending articles - March 27, 2026 (Batch 5 - Evening)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PARENT_DIR="$(dirname "$SCRIPT_DIR")"
cd "$PARENT_DIR"

echo "========================================="
echo "  Deploying 5 Trending Articles (Batch 5)"
echo "  March 27, 2026 - Evening Edition"
echo "========================================="
echo ""
...
puts "\n Step 1/4: Copying create_trending_march27_batch5.js to server..."
spawn bash -c "cat scripts/create_trending_march27_batch5.js | ssh fiscalwire 'cat > /tmp/create_trending_march27_batch5.js'"
expect {
...
spawn ssh fiscalwire "CONTAINER=\$(docker ps --filter 'name=fiscal' --format '{{.Names}}' | head -1) && echo \"Using container: \$CONTAINER\" && docker cp /tmp/create_trending_march27_batch5.js \$CONTAINER:/app/scripts/ && docker cp /tmp/add_to_zones.js \$CONTAINER:/app/scripts/ && docker exec \$CONTAINER node scripts/create_trending_march27_batch5.js"
expect {
    -re "passphrase.*:" { send "Xx123456$\r"; exp_continue }
    -re "password:" { send "Xx123456\r"; exp_continue }
    eof
