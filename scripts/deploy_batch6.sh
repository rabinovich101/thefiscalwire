#!/bin/bash
# Deployment script for Batch 6 - 15 NEW viral financial articles
# Run this ON THE PRODUCTION SERVER after SSH'ing in

echo "Deploying Batch 6: 15 NEW viral financial articles to production..."
echo ""
...
docker cp /tmp/create_15_viral_articles_batch6.js $CONTAINER:/app/scripts/
docker cp /tmp/add_to_zones.js $CONTAINER:/app/scripts/
docker cp /tmp/place_on_homepage_batch6.js $CONTAINER:/app/scripts/

echo ""
echo "Step 1: Creating 15 NEW viral articles in production database..."
docker exec $CONTAINER node scripts/create_15_viral_articles_batch6.js
...
docker exec $CONTAINER node scripts/place_on_homepage_batch6.js

echo ""
echo "Batch 6 deployment complete!"
