#!/bin/bash
# Deployment script for Batch 2 - 15 NEW viral financial articles
# Run this ON THE PRODUCTION SERVER after SSH'ing in

echo "Deploying Batch 2: 15 NEW viral financial articles to production..."
echo ""
...
docker cp scripts/create_15_viral_articles_batch2.js $CONTAINER:/app/scripts/
docker cp scripts/add_to_zones.js $CONTAINER:/app/scripts/
docker cp scripts/place_15_on_homepage.js $CONTAINER:/app/scripts/

echo ""
echo "Step 1: Creating 15 NEW viral articles in production database..."
docker exec $CONTAINER node scripts/create_15_viral_articles_batch2.js
...
docker exec $CONTAINER node scripts/place_15_on_homepage.js

echo ""
echo "Batch 2 deployment complete!"
