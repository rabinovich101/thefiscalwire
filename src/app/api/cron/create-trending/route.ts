...
const trendingArticles = [
  {
    ...
    readTime: 5,
    relevantTickers: ["SPY", "TLT", "USO", "GLD"],
...
export async function POST(request: NextRequest) {
  // Verify authentication via cron secret
  const authHeader = request.headers.get("authorization");
  const cronSecret = CRON_SECRET;

  if (!authHeader || authHeader !== `Bearer ${cronSecret}` || !cronSecret) {
  ...
  try {
    ...
    for (const article of trendingArticles) {
      ...
      let marketsCat = await prisma.category.findUnique({
        where: { slug: article.markets },
      });
      if (!marketsCat) {
        marketsCat = await prisma.category.create({
          data: {
            name: article.markets.replace(/-/g, " "),
            slug: article.markets,
            color: "bg-blue-600",
          },
        });
      }

      let businessCat = await prisma.category.findUnique({
        where: { slug: article.business },
      });
      if (!businessCat) {
        businessCat = await prisma.category.create({
          data: {
            name: article.business.replace(/-/g, " "),
      ...
      createdArticles.push({ id: newArticle.id, title: newArticle.title });
      created++;
    }

    return NextResponse.json({
      success: true,
      message: `Created ${created} trending articles for March 27, 2026`,
      articlesCreated: created,
      articles: createdArticles.map((a) => a.title.substring(0, 60) + "..."),
    });
  } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    endpoint: "/api/cron/create-trending",
    method: "POST",
    auth: "Bearer CRON_SECRET",
    description: "Creates 5 trending financial news articles for March 27, 2026",
  });
}
