...
export default async function OpinionPage() {
  const [articleData, trendingStories, pageContent] = await Promise.all([
    getCategoryArticlesWithPlacements("opinion", PAGE_SIZE, 0),
    getTrendingStories(8),
    getCategoryPageContent("opinion"),
  ]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1">
        {/* Hero Section */}
        <section className="bg-gradient-to-b from-gray-600/10 to-transparent py-12">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2 bg-gray-600 rounded-lg">
                <MessageSquare className="w-6 h-6 text-white" />
              </div>
              <h1 className="text-3xl font-bold text-foreground">Opinion</h1>
            </div>
            <p className="text-lg text-muted-foreground max-w-2xl">
              Expert commentary, market analysis, and thought-provoking perspectives from leading financial analysts.
            </p>
          </div>
        </section>
